...
input_html.addEventListener('input', function (event) {
	if (event.key === "Enter") 		{ scroll_reset(); input_submit(); stack_update(); };
	if (event.key === "ArrowUp")	scroll_up();
	if (event.key === "ArrowDown")	scroll_down();
	if (event.key === "Escape")		input_html.value = '';
	resizeTextarea(); update_cursor();
});
...
function resizeTextarea() {
	...
	console.log(parseInt(input.style.height) / 16);
}
...
function output_print(ok) {
	...
	while (output_text.length >= print_rows) output_text.shift();
